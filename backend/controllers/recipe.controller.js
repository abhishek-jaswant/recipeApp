...
exports.updateMyRecipe = async (req, res, next) => {
  ...
  try {
    // 1. Nayi images ka path nikalo aur Windows ke \ ko / mein badlo
     let updatedImages;
    if (req.files && req.files.length > 0) {
      updatedImages = req.files.map((file) => file.path.replace(/\\/g, "/"));
    }else{
       const recipe = await Recipe.findById(id);
       updatedImages=recipe.images;
    }

    // // 2. Purani images (oldImages) jo frontend se aayi hain
    // let oldImages = req.body.oldImages || [];
    // if (typeof oldImages === "string") oldImages = [oldImages];

    // // 3. Purani images ko bhi clean karo (Slashes fix karo)
    // const cleanedOldImages = oldImages.map(img => img.replace(/\\/g, "/"));

    // // 4. Dono ko merge karo
    // const finalImages = [...cleanedOldImages, ...newImages];

    // // 5. req.body se purana images data delete karo taaki conflict na ho
    const updateBody = { ...req.body };
    delete updateBody.oldImages;
    delete updateBody.images;

    // 6. DB Update
    const updated = await Recipe.findByIdAndUpdate(
      id,
      { ...updateBody, images: updatedImages },
      { new: true }
    );
